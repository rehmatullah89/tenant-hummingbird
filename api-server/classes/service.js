...
class Service {
	...
	async find(connection){
		...
		if(!service) e.th(404, "Service not found");
	...
	async create(connection, data,lease_id,  company_id, dry_run = false, property_id){

		let product = new Product({
			id: data.product_id || data.id
		});

		await product.find(connection, property_id);
		await product.verifyAccess(company_id);

		this.taxable = product.taxable;
		this.lease_id = lease_id;
		this.price = data.price || product.price;	 
		...
		// (Step#1)
		this.amount_type = product.amount_type;
		...
		if(!dry_run) await this.save(connection);
	...
	getUniqueDiscountDays(discount, period_start, period_end){
		...
		console.log("discount", discount.end);
		var end = discount.end ? moment(discount.end):  period_end;
	...
	calculateDue(connection, invoice_period_start, invoice_period_end, lease, isAnniversaryMannerDiscountPresent, payload = {}){
		const { prorate_multiplier, override_prorate = false, payment_cycle_change_dryrun = false} = payload;

		var service_period = this.getServicePeriod(invoice_period_start.clone(), invoice_period_end.clone());
		console.log("service_period", service_period)
		var billedDays = [];
		var lines = []
		...
		return models.Service.findBilledServices(connection, this.id, service_period.start.clone(), service_period.end.clone(), this.recurring)
			.then(invoiceLines => {
				...
				var BillingPeriodEnd = isAnniversaryMannerDiscountPresent ? invoice_period_end.clone() :
					 lease.getBillDateAfterDate(service_period.end.clone()).subtract(1, 'day');

				// var BillingPeriodEnd = lease.getBillDateAfterDate(service_period.end.clone()).subtract(1, 'day');
				// var BillingPeriodStart = BillingPeriodEnd.clone().add(1, 'day').subtract(1, 'month');
				...
				while (BillingPeriodEnd.isSameOrAfter(service_period.start)){
					...
					var billedDays = this.getUniqueInvoicedDays(!payment_cycle_change_dryrun ? invoiceLines : [], ActiveServiceInBillingPeriodStart, ActiveServiceInBillingPeriodEnd);

					// amount to pay for this line is (Active Service Days - Minus unique billed days ) / Total days in billing period
					var prorateMultiplier = prorate_multiplier || ((ActiveServiceDaysInBillingPeriod - billedDays.length) / DaysInBillingPeriod);
					prorateMultiplier = prorateMultiplier > 1? 1: prorateMultiplier;				

					let prorate = override_prorate || this.prorate;
	...
	async getLastBilled(connection) {

	  let lastInvoiceLine = await models.Service.hasBeenBilled(connection, this.id);
    this.last_billed = null;
    if(!lastInvoiceLine) return true;
    this.lastInvoiceLine = new InvoiceLine(lastInvoiceLine);
    this.last_billed = lastInvoiceLine.end_date;
    return true;

	}
	async getLastUnAvoidedBilled(connection, currentDate) {

		let lastInvoiceLine = await models.Service.findlastbilledInvoice(connection, this.id,currentDate);
	  this.last_billed = null;
	  if(!lastInvoiceLine) return true;
	  this.lastInvoiceLine = new InvoiceLine(lastInvoiceLine);
	  this.last_billed = lastInvoiceLine.end_date;
	...
	static async getActiveInsuranceService(connection, lease_id, start_date){

		let active = await models.Service.findActiveInsuranceService(connection, lease_id,start_date);
	...
	static async getFutureInsuranceServices(connection, lease_id, date) {

		let future_insurance_services = await models.Service.findFutureInsuranceServices(connection, lease_id, date);
		if (!future_insurance_services) return null;

		let insurance_services_list = [];

		for (let i = 0; i < future_insurance_services.length; i++) {
			const future_rent_service = new Service(future_insurance_services[i]);
			await future_rent_service.find(connection);
			insurance_services_list.push(future_rent_service);
		}

		return insurance_services_list;
	}
	...
	static async getFutureRentServices(connection, lease_id, date) {
		...
		let future_insurance_services = await models.Service.findFutureRentServices(connection, lease_id, date);
		if (!future_insurance_services) return null;
	...
	//**** * Run getLastBilled if you dont want to let them edit a billed service * ***//
	...
	async payment(payment_details, connection, res, accept_late_payments, permissions) {
		if(!payment_details || !payment_details.payment) return 
		const payment = new Payment();
		return await payment.processInBulk(connection, payment_details, res, accept_late_payments, null, {}, permissions);	
	}
	
	async updateEndDate(connection){
		await this.getLastBilled(connection);
		var save = {
			...( this.last_billed && {end_date: this.last_billed} ),
			...( !this.last_billed && {status: 0} )
		};
...
var InvoiceLine      = require('../classes/invoice_lines.js');
