...
class Document {
  ...
  static async getDocuments(company,params){
    return await pandadocs.getDocuments(company,params);
  }

  static async getLeaseDocuments(company,params){
    return await pandadocs.getLeaseDocuments(company,params);
  }

  async getTemplateDetails(company){
    // get and check values from redis first
    let redisClient = new RedisClient();
    let redisKeyName = {document_id: this.id, company_id: company.id};
    let temp_key_name = redisKeyName.document_id+"_"+redisKeyName.company_id; // just to show in the console, we can remove it from here and console below.
    let redisKeyValue = await redisClient.getValue(redisKeyName);

    if (redisKeyValue){
      this.Details = (redisKeyValue ? JSON.parse(redisKeyValue) : null);
      console.log("Getting From Redis >>>> getTemplateDetails():: -->> redisKeyName: "+temp_key_name, " => redisKeyValue: "+ redisKeyValue);
    }else{
      this.Details = await pandadocs.getTemplateDetails(this.id, company);
      redisKeyValue = JSON.stringify(this.Details);
      await redisClient.setValue(redisKeyName, redisKeyValue, process.env.REDIS_TTL_SEC); // Creating a key & Key will expire after seconds given in ENV file.
      console.log("Setting In Redis >>>> getTemplateDetails(): -->> redisKeyName: "+temp_key_name, " => redisKeyValue: "+ redisKeyValue);
    }
    // this.Details = await pandadocs.getTemplateDetails(this.id, company);
    return this.Details;
  }
            ...
            switch(key.toLowerCase()){
              ...
              case 'tenant_country':
                if(lease.Tenants[position]) {
                  if(lease.Tenants[position].Contact.Addresses.length){
                    var address = lease.Tenants[position].Contact.Addresses.filter(a => a.type == 'home');
                    mf.render = address.length? address[0].Address.country: lease.Tenants[position].Contact.Addresses[0].Address.country;
                  }
                }
                break;
              case 'tenant_zip':
              ...
              case 'tenant_full_address':
                ...
                if(lease.Tenants[position]) {
                  if(lease.Tenants[position].Contact.Addresses.length){
                    ...
                    mf.render = homeaddress.Address.zip + ' ' + (homeaddress.number ? "#" +  homeaddress.number : '') + ' ' + homeaddress.city + ' ' + homeaddress.state + ' ' + country + ' ' + homeaddress.zip;
                  }
                }
              ...
              case 'state':
                ...
                break;
              case 'country':
                mf.render = lease.Unit.Address.country || '';
                break;
              case 'zip':
              ...
              case 'number':
                ...
                break;
              case 'full_address':
                const country = lease.Unit.Address.country || ''
                mf.render = lease.Unit.Address.address + "#" + lease.Unit.number + ' ' + lease.Unit.Address.city +  ' ' + lease.Unit.Address.state + ' ' + country + ' ' + lease.Unit.Address.zip;
                break;
              case "mm/dd/yyyy":
  ...
  mergeTokens(lease){
    ...
    this.mergedTokens = pandadocs.mergeTokens( lease, this.Details);
