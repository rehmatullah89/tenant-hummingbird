...
class PropertyRateManagement {
    constructor(data) {
        let { ids: property_ids = [], company_id = '', default_unit_group_profile_id, ug_id } = data;
        ...
        let company_default_plan_id = `(SELECT crms.default_rate_management_plan FROM company_rate_management_settings crms WHERE crms.company_id = ${company_id})`

        let rate_management_plan_id = `(SELECT (IFNUll(psgtrmp.rate_management_plan_id, ${company_default_plan_id})) FROM unit_groups AS ug LEFT OUTER JOIN property_space_group_tier_rate_management_plans psgtrmp
            ON psgtrmp.tier_id = ug.unit_group_hashed_id WHERE ug.unit_group_profile_id = ${default_unit_group_profile_id} and ug.id = ${ug_id})`
        ...
        let vacant_spaces = `
            ...
            AND ug.id = ${ug_id} AND u.deleted IS NULL AND u.id NOT IN (
                ...
                AND u.id NOT IN (
                    ...
                    select unit_id from unit_status_changes where status="deactivated" and id in (
                        select max(usc.id) from unit_status_changes as usc group by usc.unit_id
                    )
                )
        ...
        this.queries = {
            ...
            // spacegroup_rent_plan_id: `SELECT 1`,
            // spacegroup_rent_plan_change_type: `SELECT 2`,
            // spacegroup_rent_plan_name: `SELECT 2`,
            // spacegroup_rent_management_active: `SELECT 2`,
            ...
            spacegroup_rate_plan_change_type: `(SELECT rmp.price_delta_type from rate_management_plans rmp WHERE rmp.id = ${rate_management_plan_id})`,

            // Exposed keys
            spacegroup_spacetype: `(SELECT CONCAT(UCASE(LEFT(ug.space_type, 1)), SUBSTRING(ug.space_type, 2))
                FROM unit_groups AS ug WHERE ug.unit_group_profile_id = ${default_unit_group_profile_id} and ug.id = ${ug_id} GROUP BY ug.id)`,

            spacegroup_rate_plan_name: `(SELECT rmp.name from rate_management_plans rmp WHERE rmp.id = ${rate_management_plan_id})`,
            ...
            spacegroup_default_promotion: `(SELECT p.name FROM unit_groups AS ug
                ...
                WHERE ug.unit_group_profile_id = ${default_unit_group_profile_id} and ug.id = ${ug_id} and pug.type = 'regular' GROUP BY ug.id)`
