...
class  AuctionDetails extends BaseReport{
  constructor(connection, company, filters, format, name, properties = [],report_name) {
    ...
    this.config.name = 'Auction Details';
    this.config.filename =  'auction_details';
  ...
  setFilterConditions(connection, conditions, structure, columns, sql_fragments ){
    if(conditions.report_period){
      this.sql_conditions += this.setTimeframes(conditions.report_period, " (select DATE(scheduled_date) from lease_auctions where deleted_at is null and lease_id = l.id having MAX(id)) ");
    }
