...
class PaymentsReport extends BaseReport{
  constructor(connection, company, filters, format, name, properties = [],report_name) {
    ...
    this.config.name = 'Credit Card Payments';
    this.config.filename = 'credit_card_payments';
    ...
    this.sql_conditions = ' WHERE p.status = 1 and p.credit_type = "payment" and p.method = "card" and (select company_id from properties where id = p.property_id ) = ' + this.company.id
