...
class PropertyRateManagementReport extends BaseReport {
    constructor(connection, company, filters, format, name, properties = [], report_name) {
        ...
        this.data = []
        this.config = {
            name: "Property Rate Management",
            filename: "property_rate_management",
            column_structure: []
                .concat(Object.values(Fields.revenue_management))
                .concat(Object.values(Fields.property_rate_management)),
            filter_structure: [],
            filters: {
                search: {
                    search: ""
        ...
        let default_unit_group_profile_id = `(SELECT default_unit_group_profile_id FROM property_rate_management_settings prms WHERE prms.property_id IN (${properties.join(',')}))`

        this.sql_tables += "unit_groups ugm"

        this.sql_conditions = `
                WHERE
                    ugm.unit_group_profile_id IN ${default_unit_group_profile_id}
        `
        this.group_sql = 'GROUP BY CONCAT(UCASE(LEFT(ugm.space_type, 1)), SUBSTRING(ugm.space_type, 2)),ugm.id'
        this.sql_fragments = Object.assign({}, new reportQueries.PropertyRateManagement({ ids: properties, company_id: company.id, default_unit_group_profile_id: default_unit_group_profile_id, ug_id: ug_id }).queries)
    }
...
module.exports = PropertyRateManagementReport
