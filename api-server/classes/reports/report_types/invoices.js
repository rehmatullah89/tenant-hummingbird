...
class  InvoicesReport extends BaseReport{
  constructor(connection, company, filters, format, name, properties = [],report_name) {
    ...
    this.config.name = 'Invoices';
    this.config.filename =  'invoices';
    ...
    this.sql_conditions = ' WHERE (inv.status = 1 or inv.voided_at is not null) and (inv.voided_at is null or DATE(inv.voided_at) > ' + connection.escape(this.report_dates.end) + ') and ' +
