...
class AllTasksReport extends BaseReport{
  constructor(connection, company, filters, format, name, properties = [],report_name) {
    ...
    this.config.name = 'All Tasks';
    this.config.filename =  'all_tasks';
...
`;
    ...
    this.sql_conditions = " where (select company_id from events where id = (select event_id from todos where id = " + todo_id + ")) =  " + this.company.id 
