...
class PaymentsReport extends BaseReport{
  constructor(connection, company, filters, format, name, properties = [],report_name) {
    ...
    this.config.name = 'Cash Payments';
    this.config.filename = 'cash_payments';
    ...
    this.sql_conditions = ' WHERE p.status = 1 and p.credit_type = "payment" and p.method = "cash" and (select company_id from properties where id = p.property_id ) = ' + this.company.id
