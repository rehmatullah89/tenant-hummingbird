...
class RentRollReport extends BaseReport{
  constructor(connection, company, filters, format, name, properties = [],report_name) {
    ...
    this.config.name = 'Rent Roll';
    this.config.filename =  'rent_roll';
    this.config.key =  'rent_roll';
  ...
  setFilterConditions(connection, conditions, structure, columns, sql_fragments ){
    if(conditions.report_period){
      let period = this.getTimeframeDates(conditions.report_period);
      this.sql_conditions +=` AND (l.start_date <= '${period.end}' ) 
                              AND (l.end_date is null OR ( DATE(l.end_date) >= '${period.start}' ))`;
    }
  }
