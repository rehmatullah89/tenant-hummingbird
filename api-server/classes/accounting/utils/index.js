...
module.exports = {
  generateFileName(params) {
    ...
    if(format){
        ...
        if(['csv_quickbooks','csv_yardi', 'csv_yardifinancialjournal','csv_yardi-financial-journal-ipp'].indexOf(format) > -1){
            filename += `-${format_split[1]}`;
        } else if(['csv_sageintacct', 'pdf_accountant_summary', 'pdf_balance_sheet','csv_charges_detail','csv_charges_journal','pdf_charges_journal'].indexOf(format) > -1){
            filename = `${format_split[1]}`;
        }
    }
