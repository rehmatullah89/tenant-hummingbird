...
class BaseExport {
  ...
  static async InitAccountingExport(connection, data){
    ...
    if(!data.Property) {
      const property = new Property({ id: data.property_id, company_id: data.company_id });
    ...
    data.credit_debit_account = creditDebitAccounts;
      ...
      if(eventClass){
        data.uuid = uuidv4();
        let off_set = await data.Property.getUtcOffset(connection);
        data.export_date = moment().utcOffset(parseInt(off_set)).format('YYYY-MM-DD');
