...
class TsysCard extends PaymentMethod {
  ...
  //   // await getTransactionDetails(this.connection_info, payment, this.cc_token, this.token, company_id)
  ...
  //   }
    ...
    body.CardAuthentication.expirationDate = this.exp_yr.length === 4? this.exp_mo + "/" + this.exp_yr : this.exp_mo + "/" + moment(this.exp_yr, "YY").format("YYYY");
  ...
  async void(connection, payment, company_id, amount, refund){
    ...
    return {
      ...
      status: response.VoidResponse.status
    }
  ...
  async return(connection, payment, company_id, amount, refund){
    ...
    if(response.ReturnResponse.status === 'FAIL'){
      e.th(400, "Issuer Rejection: " + response.ReturnResponse.responseMessage);
    }
  ...
  async charge(connection, amount, payment_id, company_id, dryrun, contact, payment_source, authorization = null, refund_effective_date, ip_override, paymentRef = {}){
    ...
    try {
      ...
      if(!this.token)
        this.token=this.card_number;

      if(this.token){
        try{
          // attempt to charge card
          response = await this.chargeToken(this.connection_info, amount, this.token, payment_id, company_id, contact && contact.roles.includes('admin'), payment_source);
        } catch(err){
          error = err;
        }
      } else {
      ...
      if(!error){
        ...
        payment_info.status_desc = error;
      ...
      if(response && response.responseMessage === 'Partially Approved' ){
        ...
        const propertyDate = this.property_id ? await property.getLocalCurrentDate(connection, 'YYYY-MM-DD hh:mm:ss') : moment.utc().format('YYYY-MM-DD HH:mm:ss');
        let refund = {
          type: 'void',
          amount: response.processedAmount,
      ...
      else if(response && response.responseMessage && response.responseMessage === 'Request Failed'){
        const property = new Property({ id: this.property_id });
        const propertyDate = this.property_id ? await property.getLocalCurrentDate(connection, 'YYYY-MM-DD hh:mm:ss') : moment.utc().format('YYYY-MM-DD HH:mm:ss');        
        let refund = {
          type: 'refund',
        ...
        if (refund_response.status != "FAIL"){
          refund.auth_code = refund_response.auth_code;
          refund.transaction_id = refund_response.transaction_id;
          await models.Payment.saveRefund(connection, refund);
          clsContext.push(Enums.EVENTS.GENERATE_EVENT_EXPORT, { event_id: Enums.ACCOUNTING.EVENTS.REFUNDS, payment_id: payment_id, property_id: this.property_id });
        }
      }
      if(error) e.th(400, error.msg);
    } catch(err){
      console.log("TSYS Card",err.stack);
      let payment_info = {
        payment_gateway: this.payment_gateway,
  ...
  async chargeToken(connection_info, amount, token, payment_id, company_id, is_admin, payment_source =  'e-commerce'){
    ...
    if(this.card_type.toLowerCase() === "mastercard" && this.token_alt && is_mit) {
      body.Sale.mitStatusIndicator = "M101";
    }
    ...
    try {
      ...
      this.log('GatewayError: ' + err.message);
      e.th(400, 'GatewayError: Unexpected Gateway Error, please try again later.');
