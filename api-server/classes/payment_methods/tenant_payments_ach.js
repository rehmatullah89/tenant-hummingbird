...
let uri = process.env.TENANT_PAYMENTS_ENDPOINT;
...
class ProPayAch extends PaymentMethod {
  ...
  static async submitEquipmentPurchaseRequest(data) {


    try {

      let phoneNumber = data.MailAddress.Phone.replace(/\D/g, '');
      let ccNumber = data.CreditCardData.CreditCardNumber.replace(/\D/g, '');
      ...
      let xml = `<?xml version='1.0'?>
        ...
        <XMLRequest>
          ...
          <XMLTrans>
            ...
            <accntNum>${data.AccountNumber}</accntNum>
            <shipTo>${data.ShipTo}</shipTo>
            <shipToContact>${data.shipToContact}</shipToContact>
            <shipToAddress>${data.MailAddress.Address1}</shipToAddress>
            <shipToAddress2>${data.MailAddress.Address2}</shipToAddress2>
            <shipToCity>${data.MailAddress.City}</shipToCity>
            <shipToState>${data.MailAddress.State}</shipToState>
            <shipToCountry>${data.MailAddress.Country}</shipToCountry>
            <shipToZip>${data.MailAddress.Zip}</shipToZip>
            <shipToPhone>${phoneNumber}</shipToPhone>
            <cardholderName>${data.CreditCardData.NameOnCard}</cardholderName>
            <CcNum>${ccNumber}</CcNum>
            <ExpDate>${data.CreditCardData.ExpirationDate}</ExpDate>
            <CVV2>${data.CreditCardData.CVV}</CVV2>
            <billingZip>${data.CreditCardData.CardZip}</billingZip>
            <Devices>
               <Device>
                  <Name>${data.Device.Name}</Name>
                  <Quantity>1</Quantity>
                  <Attributes/>
               </Device>
            </Devices>
      ...
      const parser = new XMLParser();
      let r = parser.parse(response);
      console.log("submitEquipmentPurchaseRequest Response", r.XMLResponse);

      return {
        status: 'OrderDevice',
        status_code: r.XMLResponse.XMLTrans.status,
        ref_num: null,
        auth_code: null,
        transaction_id: r.XMLResponse.XMLTrans.transNum,
      }
  ...
  async chargeACH(connection_info, amount, company_id, payment_source = 'e-commerce') {
    ...
    try {
      ...
      switch (r.XMLResponse.XMLTrans.status) {
        ...
        default:
          e.th(400, Enums.TENANT_PAYMENTS.API_RESPONSE_ERROR_CODES[r.XMLResponse.XMLTrans.status]);
      }
