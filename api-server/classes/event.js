...
class Event {
	...
	async findObject(connection){
		...
		switch (res.object_type) {
			case Enums.TASK_TYPE.LEASE:
				...
				await this.Lease.getProperty(connection);
				await this.Lease.findAuction(connection, this.Lease.Property.company_id);
				if(this.Lease.created_by){
					this.Created = new Contact({id: this.Lease.created_by});	
					await this.Created.find(connection, this.company_id);
					await this.Created.getRole(connection, this.company_id);
				} 
			...
			case Enums.TASK_TYPE.CONTACT:
				...
				this.Created = new Contact({id: this.created_by});
				await this.Created.find(connection, this.company_id);
				await this.Created.getRole(connection, this.company_id);
				break;
			case Enums.TASK_TYPE.RATE_CHANGE:
				...
				await this.Rate_Change.findRentChangeLeases(connection);
                await this.Rate_Change.getStats(connection);
				break;
		}
  ...
  async findEventUploadCount(connection){
	...
	async create(connection, type, data, user_id, company){
		...
		this.end_date = data.end_date ?  moment.utc(data.end_date).format('YYYY-MM-DD HH:mm:ss') : null;
  ...
  setTrigger(company){
	...
	// ping(){
	// 	if(settings.is_prod){
	// 		return rp("https://api.leasecaptain.com/v1/todos/ping/" + this.company_id);
	// 	} else {
	// 		return rp("http://10.0.46.14/v1/todos/ping/" + this.company_id);
	// 	}
	// }
  ...
  static async findEventTypeById(connection, id){
	
    return await models.Event.findEventTypeById(connection, id);
  ...
  async setEventEndDate(connection, params) {
	if(this.event_type){
		...
		if(!this.end_date && this.event_type.expiration_days){
