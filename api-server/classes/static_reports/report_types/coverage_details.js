...
class CoverageDetails extends BaseStaticReport {
    ...
    async getData() {
        ...
        if (!this.end_date || !moment(this.end_date, 'YYYY-MM-DD', true).isValid()) e.th(400, "Please enter an end date for this report ");

        let data = await Report.coverageDetails(this.connection, [this.property_id], this.company.id, this.date, this.end_date);
