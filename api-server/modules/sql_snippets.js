...
module.exports = {
  ...
  unit_status(unit_id, date, time){
    ...
    return " (SELECT IF( " +
          " (select status from unit_status_changes where unit_id = " + unit_id + " and status in ('activated','deactivated') and (select DATE(CONVERT_TZ(created_at,'+00:00',IFNULL(utc_offset,'+00:00'))) from properties where id = (select property_id from units where id = " + unit_id + ")) <= " + date + " order by id desc limit 1 ) = 'deactivated', " +
          " 'Deactivated', " +
            "IF( " +
            " (SELECT id from units where id = " + unit_id + " and id not in (select unit_id from overlocks where status = 1) and id in (select unit_id from leases where status = 1 and start_date <= " + date + " and (end_date is null or end_date > " + date + ") and to_overlock = 1)), " +
            " 'To Overlock'," +
              "IF( " +
              " (SELECT id from units where id = " + unit_id + " and id in (select unit_id from overlocks where unit_id=" + unit_id + " and status = 1) and id in (select unit_id from (select id, unit_id,to_overlock from leases where unit_id=" + unit_id + " and status = 1 and start_date <= " + date + " and (end_date is null or end_date >= CURDATE()) order by id desc limit 1) as x where to_overlock = 0)), " +
              " 'Remove Overlock'," +
                "IF( " +
                  " (SELECT id from units where id = " + unit_id + " and id in (select unit_id from overlocks where status = 1) ), " +
                  " 'Overlocked'," +
                      ...
                      " IF( " +
                        ...
                        " (SELECT id from leases where start_date > " + date + " and unit_id = " + unit_id + " and (end_date > " + date + " or end_date is null ) and status = 1 HAVING MAX(id) ) IS NOT NULL, " +
                          ...
                          " IF( " +
                            ...
                            " 'On Hold', " +
                            " IF( " +
                              " ((select status from unit_status_changes where unit_id = " + unit_id + " and date <="+ date +" and status in ('online','offline')  order by id desc limit 1) = 'offline' or (select available_date from units where id = " + unit_id + " )  > " + date + "), " +
                              " 'Offline', " +
                                " 'Available' " +
                              " ) " +
                            " ) " +
  ...
  // TODO add Overlocked
  unit_status_lease(unit_id, lease_id, date, time){
    date = date ? `" ${date} "` : 'CURDATE()';
    facility_time = time ? `" ${time} "` : `(select CONVERT_TZ(now(),"+00:00",utc_offset) from properties where id = (select property_id from units where id = ${unit_id}))`;
    time = time ? `" ${time} "` : 'now()';

    return " (SELECT IF( " +
          " (select status from unit_status_changes where unit_id = " + unit_id + " and status in ('activated','deactivated') order by id desc limit 1 ) = 'deactivated', " +
          " 'Deactivated', " +
            "IF( " +
            " (SELECT id from units where id = " + unit_id + " and id not in (select unit_id from overlocks where status = 1) and id in (select unit_id from leases where status = 1 and id = "+ lease_id +" and start_date <= " + date + " and (end_date is null or end_date > " + date + ") and to_overlock = 1)), " +
            " 'To Overlock'," +
              "IF( " +
              " (SELECT id from units where id = " + unit_id + " and id in (select unit_id from overlocks where status = 1) and id in (select unit_id from leases where status = 1 and id = "+ lease_id +" and start_date <= " + date + " and (end_date is null or end_date >= CURDATE()) and to_overlock = 0)), " +
              " 'Remove Overlock'," +
                "IF( " +
                  " (SELECT id from units where id = " + unit_id + " and id in (select unit_id from overlocks where status = 1) ), " +
                  " 'Overlocked'," +
                  "IF( " +
                    " (SELECT id from leases where start_date <= " + date + " and (end_date > " + date + " or end_date is null ) and status = 1 and unit_id = " + unit_id + " and id = "+ lease_id +" HAVING MAX(id) ) IS NOT NULL, " +
                    " 'Leased', " +
                    " IF( " +
                      " (SELECT id from leases where status = 2 and id = "+ lease_id +" and unit_id = " + unit_id + " HAVING MAX(id) ) IS NOT NULL, " +
                      " 'Pending', " +
                      " IF( " +
                        " (SELECT id from leases where (end_date > " + date + " or end_date is null ) and id = "+ lease_id +" and status = 0 and unit_id = " + unit_id + " and id in ( SELECT lease_id from reservations where expires >= " + facility_time + " ) HAVING MAX(id) ) IS NOT NULL, " +
                        " 'Reserved', " +
                        " IF( " +
                        " (SELECT id from leases where start_date > " + date + " and id = "+ lease_id +" and unit_id = " + unit_id + " and (end_date > " + date + " or end_date is null ) and status = 1 HAVING MAX(id) ) IS NOT NULL, " +
                          " 'Future Leased', " +
                          " IF( " +
                            " ( select unit_id from unit_holds where expires > " + time + " and unit_id = " + unit_id + ") IS NOT NULL, " +
                            " 'On Hold', " +
  ...
  unit_amenity(amenity_name, amenity_category_id){
    return `(select distinct au.value from amenity_units au where unit_id = u.id and amenity_property_id in (select id from amenity_property  where property_id = u.property_id and amenity_category_id = ${amenity_category_id} and property_type = u.type and amenity_name = '${amenity_name}') )`
  },
    ...
    let query = `
      (cast(
        (
          ...
          from units un where un.id = ${unit_id}
        )
      as decimal(10,2)))
    `;
    return query;
  ...
  lease_rent(lease_id, date) { 
    let query = `select price from services s where product_id in (select id from products where default_type = 'rent') and lease_id = ${lease_id} and start_date <= ${date} and ( end_date is null or end_date >= ${date} ) Having MAX(start_date) `;

    return query;
  },

  /**
   * This is exactly the copy of the above 'lease_rent' snippet
   * as that contains some issues with checking lease status
   */
  get_current_rent_of_lease(lease_id, date) {
    // Fallback to current date if none passed
    date = date || 'CURDATE()'
    ...
    let query = `
      SELECT s.price FROM services s
        JOIN products p ON
          ...
          p.default_type = 'rent'
        WHERE
          s.lease_id = ${lease_id} AND
          s.status = 1 AND
          s.start_date <= ${date} AND
          (
  ...
  getFractionalDateDifference(startDate, endDate, differenceType) {
    let query = `
      ROUND((TIMESTAMPDIFF(${differenceType}, ${startDate}, ${endDate}) +
        DATEDIFF(
          ${endDate},
          ${startDate} + INTERVAL TIMESTAMPDIFF(${differenceType}, ${startDate}, ${endDate}) ${differenceType}
        ) /
        DATEDIFF(
          ( ${startDate} + INTERVAL TIMESTAMPDIFF(${differenceType}, ${startDate}, ${endDate}) + 1 ${differenceType} ),
          ( ${startDate} + INTERVAL TIMESTAMPDIFF(${differenceType}, ${startDate}, ${endDate}) ${differenceType})
        )),
      2)
    `
  ...
  getColumnNames(table, exclude_columns = []) {

    exclude_columns.push("id");
    console.log(exclude_columns.map(i => `'${i}'`).join(', '));

    let query = `
      SELECT DISTINCT COLUMN_NAME
      FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_NAME = '${table}' AND TABLE_SCHEMA = '${process.env.MYSQL_DATABASE}'
        AND COLUMN_NAME NOT IN (${exclude_columns.map(i => `'${i}'`).join(', ')});
    `
    console.log("getColumnNames columns_to_copy :", query);
    return query;
  }
