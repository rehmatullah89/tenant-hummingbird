...
var queryOptions = {
	...
	storageTypeOptions:         ["Self Storage", "Wine Storage", "Locker", "Outdoor Space", "Cold Storage", "Parking"],
...
class UnitSearcher {
	...
	constructor(company_id, unit_type) {
		...
		this.sql_count_base = "Select * from units where 1 = 1 and type = '" + unit_type + "' and  (select company_id from properties where id = units.property_id ) = " + company_id;
		this.sql_order = '';
		this.sql = '';
		this.count_sql = '';
	}
	...
	build_amenity_conditions_from_array(connection, label, optionLabel, chosen){
		...
		if(optionLabel) {
			...
			var filtered = chosen.filter(c => c || c === 0 || c === '0');
		}

		if(!filtered.length) return;

		filtered.forEach( c => {
			this.build_conditions(' (select value from amenity_units where unit_id = units.id and amenity_property_id = (select id from amenity_property where lower(amenity_name) = ' + connection.escape(label) +' and property_type = ' + connection.escape(this.unit_type) + ' and property_id=units.property_id)) = ' + connection.escape(c.toLowerCase()) + ' OR');
		});
		this.trim_query(2);
	...
	build_amenity_conditions_gt(connection, label, min){
		this.build_conditions(' and (');
		this.build_conditions(' (select value from amenity_units where unit_id = units.id and amenity_property_id = (select id from amenity_property where lower(amenity_name) = ' + connection.escape(label) +' and property_type = ' + connection.escape(this.unit_type) + ' and property_id=units.property_id)) >= cast(' + connection.escape(min) + " as decimal(5,2))" );
	...
	build_meta_query(){
		...
		this.sql_base = " select count(*), " +
			" (select MAX(price) ) as max_price, " +
			" (select MIN(price) ) as  min_price, " +
			" (select MIN(available_date) ) as min_available, " +
			" (select MAX(available_date) ) as max_available, " +
			" (select COUNT(id) where status = 1 ) as count_online, " +
			" (select COUNT(id) where status = 0 ) as count_offline, " +
			" (select COUNT(id) where featured = 0 ) as count_not_featured, " +
			" (select COUNT(id) where featured = 1 ) as count_featured, " +
			" (select COUNT(DISTINCT(category_id) )) as category_counts " +
			" FROM units WHERE 1 = 1 and type = '" + this.unit_type + "'and (select company_id from properties where id = units.property_id ) = " + this.company_id;
	}
	...
	set_status_conditions(status_list){
		...
		if(status_list.indexOf('on hold') >= 0) this.set_condition_on_hold();
	...
	set_condition_pending(){
		this.sql_conditions += " id in ( (select unit_id from leases where " +
			" ( end_date > '" + moment().format('YYYY-MM-DD') + "' or end_date is null ) and " +
			" status = 2 )) or ";
	}
	...
	set_condition_reserved(){
		this.sql_conditions += " id in ( ( select unit_id from leases where status = 0 and start_date <= '" + moment().format('YYYY-MM-DD') + "' and " +
			" ( end_date > '" + moment().format('YYYY-MM-DD') + "' or end_date is null ) and id in ( select id from reservations where expires > now() ) ) ) or ";
	}

	set_condition_on_hold(){
		this.sql_conditions += " id in ( ( select unit_id from leases where status = 0 and start_date <= '" + moment().format('YYYY-MM-DD') + "' and  ( end_date > '" + moment().format('YYYY-MM-DD') + "' or end_date is null ) and id in ( select id from unit_holds where expires > now() ) ) ) or ";
	}
	...
	query_amenity(name){
		this.sql_base += ", (select value from amenity_units where unit_id = units.id and amenity_property_id = (select id from amenity_property where lower(amenity_name) = '" + name.toLowerCase() + "' and property_type = '" + this.unit_type + "' and property_id=units.property_id)) as " + name.toLowerCase();
	}

	query_image(){

		this.sql_base += ", (select name from uploads INNER JOIN uploads_units on uploads.id = uploads_units.upload_id where uploads_units.unit_id = units.id and uploads.document_type_id in (select id from document_types where name = 'image') and uploads_units.status = 1 order by uploads_units.sort ASC, uploads_units.id ASC limit 1 ) as unit_image_filename, " +
			" (select uploads.id from uploads INNER JOIN uploads_units on uploads.id = uploads_units.upload_id where uploads_units.unit_id = units.id and uploads.document_type_id in (select id from document_types where name = 'image') and uploads_units.status = 1 order by uploads_units.sort ASC, uploads_units.id ASC limit 1 ) as unit_image_id ";
	...
	sort_numeric(field, dir){
		this.sql_order += field + '* 1 ' + dir + ',  units.available_date ASC '
	}
	...
	sort_unit_number(dir){
		...
		this.sql_order += ' address ' + dir + ', LPAD(LOWER(number), 10,0) ' + dir;
	}
