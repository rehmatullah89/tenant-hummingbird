module.exports = {
	...
	findDetails(connection, payload = {}) {
		...
		const { company_id, base_properties = [], restricted_properties = [] } = filters;

		let sql = `
			select t.*, s.id as accounting_setup_id, 
				(select group_concat(pa.property_id) from property_accounting_template pa where pa.accounting_template_id = t.id and pa.deleted_at is null ${restricted_properties.length ? ('and pa.property_id in ( ' + restricted_properties.map(r => connection.escape(r)).join(',')) + ' ) ': (base_properties.length ? ('and pa.property_id in ( ' + base_properties.map(r => connection.escape(r)).join(',') + ' )' ) : '')}) as property_ids 
	...
	updateTemplateDefaultSubTypeAccounts(connection,payload){
		...
		return connection.queryAsync(sql, [
            payload.map(p => [
                ...
                p.gl_account_id ? connection.escape(p.gl_account_id): null, 
