...
module.exports = {
  ...
  findEventObject: function(connection, event_id){
    var sql = "SELECT * FROM event_objects where event_id =  " + connection.escape(event_id);
    console.log("sql", sql)
    return connection.queryAsync(sql).then(a => a.length ? a[0] : null );
  },
  ...
  findContactEvent: function (connection, event_id) {
    var sql = "SELECT * FROM event_contacts where event_id =  " + connection.escape(event_id);
    return connection.queryAsync(sql).then(a => a.length ? a[0] : null );
  },
  ...
  saveEventType(connection, data, event_type_id){
      var sql = '';
      if(event_type_id){
        sql = "update event_types set ?  where id = " + connection.escape(event_type_id);
      } else {
        sql = "insert into event_types set ?";
      }

    return connection.queryAsync(sql, data).then(r => event_type_id ? event_type_id : r.insertId);
  },
  ...
  findEventTypes(connection, type){
