...
module.exports = {
	...
	async save (connection, data, activity_id) {
	...
	findAdminActivity(connection, conditions, searchParams, count){
		...
		console.log("findAdminActivity sql", sql);

		return connection.queryAsync(sql).then(r => console.log("r", r) || r);
	...
	findContactActivity(connection, params){
		...
		var sql = "SELECT * from activity where 1 = 1 and contact_id  = " + connection.escape(params.contact_id) + " or (object_id = " + connection.escape(params.contact_id) + " and activity_object_id in (select id from activity_objects where interaction = 1)  ) order by created desc ";
	...
	findContactInteractions(connection, contact_id, conditions = {}, searchParams, count){
		...
		sql += " from activity where 1 = 1 and activity_object_id in (select id from activity_objects where interaction = 1) and object_id = " + connection.escape(contact_id);
	...
	async duplicateActivities(connection, payload) {
