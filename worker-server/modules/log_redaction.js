...
var RM = {
    redactSendSMS(logs = {}){
        ...
        if (logs.request?.messages?.length) {
            logs.request.messages = "*";
        } 
        
        
        if (logs.response?.provider_response?.body?.length) {
            logs.response.provider_response.body = RM.methods.text(logs.response.provider_response.body);
        } 
        
        if (logs.response?.provider_response?.to?.length) {
            logs.response.provider_response.to = RM.methods.text(logs.response.provider_response.to);
        } 
        

        if (logs.response?.provider_response?.from_?.length) {
            logs.response.provider_response.from_ = RM.methods.phoneNumber(logs.response.provider_response.from_);
        } 

       
        return logs
    }, 
    redactSendEmail(logs = {}){

        if (logs.request?.message?.length) {
            logs.request.message = "*";
        } 

        if (logs.request?.to?.length) {
            logs.request.to = "*";
        } 
    ...
    redactGateAccessAppLogs(logs = {}){
        if (logs.results?.data?.user) {
            logs.results.data.user =  RM.objects.contact(logs.results.data.user)
            logs.results.data.user.phone = RM.methods.phoneNumber(logs.results.data.user)
        } 
        return logs; 
    },

    redactGenerateShareReports(logs = {}){
        logs.payload.reports = '****'
