...
class Accounting {
  ...
  async generateExport(connection, payload) {
    ...
    const property = new Property({ id: payload.property_id });
    ...
    if (export_data && export_data.length > 0) {
      ...
      await gl_export.bulkSave(connection, export_data, books, event.uuid, event.property_id, payload.source);

      console.log("Accounting Export Finished");
    }
  }

  async setIsFutureInvoice(connection, { invoiceId, date }) {
    let invoice = new Invoice({ id: invoiceId });
    const invoiceTimeSpan = await invoice.getTimeSpan(connection, { date });
  ...
  async isRefund(connection, invoicesPaymentsBreakdownId){
    ...
    return invoice_payment_breakdown && invoice_payment_breakdown[0].refund_id ? true : false
  }
  ...
  async computeAccountingData(connection, payload) {
    ...
    const { cid, invoiceId, paymentId, propertyId, invoicesPaymentsBreakdownId, date, source } = payload;   
  ...
  static async createDefaultAccountingTemplateForCompany(connection, payload){
    console.log("Creating Default Template");
    return await models.Accounting.createDefaultAccountingTemplateForCompany(connection,payload);
  }
  static async assignTemplateToAccountingBook(connection, payload){
    console.log("Updating Accounting Setup");
    await models.Accounting.assignTemplateToAccountingBook(connection,payload);
  }
  static async assignTemplateToGlEvents(connection, payload){
    console.log("Updating GL Events");
    await models.Accounting.assignTemplateToGlEvents(connection,payload);
  }
  static async assignTemplateAndAccountsToDefaultSubTypes(connection, payload){
    console.log("Adding Default SubTypes");
    await models.Accounting.assignTemplateAndAccountsToDefaultSubTypes(connection,payload);
  }
  static async assignCorporateAccountToPropertyProducts(connection, payload){
    console.log("Updating Property Products");
    await models.Accounting.assignCorporateAccountToPropertyProducts(connection,payload);
  }
  static async assignDefaultTemplateToAllPropertiesOfCompany(connection, payload){
    console.log("Adding Properties to Template");
    await models.Accounting.assignDefaultTemplateToAllPropertiesOfCompany(connection,payload);
  }
  static async assignAccountingToggleSettingsToCompany(connection, payload){
    console.log("Adding Accounting Settings");
    ...
    await models.Accounting.assignAccountingToggleSettingsToCompany(connection,settingsObj);
  }

  static async generateInvoicesExports(connection, { company_id, property_id, from_date, to_date }) {
    const invoices = await models.Invoice.getAllInvoices(connection, {
      company_id,
      property_id,
      from_date,
      to_date
  ...
  static async generateVoidInvoicesExports(connection, { company_id, property_id, from_date, to_date }) {
    const invoices = await models.Invoice.getAllVoidInvoicesToGenerateEvent(connection, {
      company_id,
      property_id,
      from_date,
      to_date
  ...
  static async generateAllowanceExports(connection, { company_id, property_id, from_date, to_date }) {
    const payments = await models.Payment.getAllCreditPayments(connection, {
      company_id,
      property_id,
      from_date,
      to_date
  ...
  static async generatePaymentsExports(connection, { company_id, property_id, from_date, to_date, breakdown_ids }) {
    const payments = await models.Payment.getAllPaymentsBreakdown(connection, {
      company_id,
      property_id,
      from_date,
      to_date,
  ...
  static async generateRevenueRecognitionExports(connection, { company_id, property_id, from_date, to_date, invoice_id }) {
    const invoices = await models.Invoice.advancePaidInvoices(connection, {
      company_id,
      property_id,
      from_date,
      to_date,
  ...
  static async generateRefundsExports(connection, { company_id, property_id, from_date, to_date, refund_ids }) {
    const refunds = await models.Payment.getAllRefunds(connection, {
      company_id,
      property_id,
      from_date,
      to_date,
  ...
  static async generateInterPropertyPaymentExportsExports(connection, { company_id, property_id, from_date, to_date, refund_ids }) {
    const interPropertyPayments = await models.Payment.getAllInterPropertyPayments(connection, {
      company_id,
      property_id,
      from_date,
      to_date
    ...
    return interPropertyPayments;
  }
  
  static async genrateGLExports(connection, payload) {
    let events = [];

    const invoiceExports = await Accounting.generateInvoicesExports(connection, payload);
    const allowanceExports = await Accounting.generateAllowanceExports(connection, payload);
    const paymentsExports = await Accounting.generatePaymentsExports(connection, payload);
    const revenueExports = await Accounting.generateRevenueRecognitionExports(connection, payload);

    events = [...invoiceExports, ...allowanceExports, ...paymentsExports, ...revenueExports];
    return events;
  }
  ...
  async getContactTransactions(connection, params) {
    ...
    if(lease_id){
      ...
      leases = await models.Accounting.getLeasesByContact(connection, { contact_id });
      ledger_result = await models.Accounting.getLedgerByContact(connection, contact_id, property_id, { start_dt: from_date, end_dt: to_date });
    }
  ...
  static async findMissingJournalEventConfig(connection, payload){
    let { company_id, property_id } = payload;

    if(!property_id) {
      return;
    }

    let cash_events = [], accrual_events = [];

    const property = new Property({ id: property_id, company_id: company_id });
    let accounting_setup = await property.findAccountingSetup(connection);
    if(!accounting_setup) return null;

    let missing_journal_events = await models.Accounting.findMissingJournalEventConfig(connection, { accounting_template_id: property.PropertyAccountingTemplate.id });
    missing_journal_events.forEach(je => parseInt(je.book_id) == 0 ? cash_events.push(je.name) : accrual_events.push(je.name));

    return this.missingEventsStructure(parseInt(accounting_setup.book_id), cash_events, accrual_events);
  }

  static missingEventsStructure(book, cash_events, accrual_events){
    if(book == 0)
      return [{book: 'Cash Book', events: cash_events}]
    if(book == 1)
      return [{book: 'Accrual Book', events: accrual_events}]
    if(book == 2) 
      return [{book: 'Cash Book', events: cash_events}, {book:'Accrual Book', events: accrual_events}]
  }
