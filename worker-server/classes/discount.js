...
class Discount {
    ...
    // async terminate(connection, date){
    //     if(!this.id) e.th(400, "Missing id");
    //     return await models.Discount.end(connection, this.id, date);  
    // }
    ...
    validate(){
        ...
        try {
            ...
            if (!_this.promotion_id) {
                throw 'Promotion id missing';
            }

            if (!_this.start || !validator.isDate(_this.start + '')) {
                throw 'Start must be a date';
            }

            if (_this.end && !validator.isDate(_this.end + '')) {
            ...
            // if (!_this.value || !validator.isDecimal(_this.value + '')) {
            //     throw 'Discount value is missing';
            // }

            if (_this.type !== 'dollar' && _this.type !== 'percent' && _this.type !== 'fixed') {
