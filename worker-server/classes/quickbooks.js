...
class QuickBooks {
    ...
    init(connection){
        ...
        return _this.findSettings(connection)
            .then(function(userSettingsRes){
                if(Object.keys(userSettingsRes).length === 0 && userSettingsRes.constructor === Object) throw "QuickBooks Not Configured";
                return _this.connect(userSettingsRes);

            }).then(function(company_file){
                _this.company_file = company_file;
        ...
        return models.Setting.findSettings(connection, 'quickbooks', this.company_id)
            .then(function(settingsRes){
                settingsRes.forEach(function(setting){
                    ...
                    var connectionVals = ['oauthTokenSecret', 'oauthToken', 'realmId'];
    ...
    connect(userSettings){
        ...
        var qbo = new QB(settings.quickbooks.consumer_key,
            ...
            userSettings['oauthToken'],
            userSettings['oauthTokenSecret'],
            userSettings['realmId'],
        ...
        return _this.qboPromise.getCompanyInfoAsync(_this.user_settings['realmId']).then(function(companyRes){
    ...
    listAccounts(criteria){
        ...
        return _this.qboPromise.findAccountsAsync(criteria).then(function(qbAccountsRes){
            return qbAccountsRes.QueryResponse.Account;
        }).catch(function(err){
            console.log(err);
            _this.msg = err.toString();
            return [];
    ...
    listTaxRates(){
        ...
        return _this.qboPromise.findTaxCodesAsync().then(function(qbTaxCodesRes){
            
            return qbTaxCodesRes.QueryResponse.TaxCode;
        }).catch(function(err){
            console.log(err);
            _this.msg = err.toString();
            return [];
    ...
    savePayment(connection, payment){
        ...
        return Promise.resolve().then(function() {
            ...
            return models.Payment.save(connection, { qb_id: qbPayment.Id }, payment.id);
