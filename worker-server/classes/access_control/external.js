...
class External extends AccessControl {
	...
	async addContactToSpace(connection, contact, creds, unit_id, property_id){
		...
		try{
			console.log('URL', this.endpoint + 'facilities/'  + property_id + '/spaces/'  + unit_id + '/users/' + contact.id);
	...
	async generateCode(){
		...
		while(!validCode) {
			...
			return code;
			validCode = await this.validateCode(code);
		}
	...
	async suspendUser(connection, contact, creds, lease){
		var user = {};
	...
	async updateUserSpace(connection, contact, creds, lease){
		...
		if(has_access && user_exists) {
			// update users
			await this.updateContactToSpace(connection, contact, creds, lease.Unit.id, lease.Property.id)
