...
class Forte extends PaymentMethod {
	...
	async charge(connection, amount, payment_id, company_id, dryrun, contact, source){
	  ...
	  try {
		  ...
		  console.log("DETAILS FOR PAYMENTS");
		  console.log(this.connection_info);
		  console.log(this.ach_token);
	...
	getPaymentStatus(connection, payment, company_id){
		return this.setCustomerProfileToken(connection, company_id)
			.then(() => {
				...
				return forteACHFuncs.getTransaction(this.connection_info, payment, this.ach_token, this.token, company_id)
					.catch(err => {
						e.th(400, err.error.response.response_desc);
					})
					.then(body => {
						...
						switch (body.status.toLowerCase()) {
							...
							case 'funded':
								return {
									status_desc: body.response.response_desc,auth_code: body.response.authorization_code,
									status: 1
								}
							default:
								return null;
						...
						if(!status) return;
						return models.Payment.save(connection, status, payment.id).then(() => {
							if(status.status) return;
							return models.Payment.unapplyByPaymentId(connection, payment.id)
						})
	...
	console.log("contact.Token", contact.Token);
