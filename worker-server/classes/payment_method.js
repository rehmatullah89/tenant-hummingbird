...
class PaymentMethod {
    ...
    async find(connection) {
        ...
        this.card_end = data.card_end ? data.card_end.substr(data.card_end.length - 4, data.card_end.length) : null;
        this.card_type = data.card_type;
    ...
    async makeAutoPayment(invoice){

        if(!this.AutoPay) return;

        let rent_payment = 	Math.round(this.AutoPay.rent * invoice.rent_total) / 1e2;
        let other_payment = Math.round(this.AutoPay.other * invoice.utilities_total) / 1e2;

        let amount = Math.round( (rent_payment + other_payment) * 1e2) / 1e2;
        ...
        // invoice.rent_total -=  rent_payment;
        // invoice.utilities_total -=  other_payment;
    ...
    async findLeasesToAutoPay(connection, date){
        return await models.Payment.findAutoPayByPaymentId(connection, this.id, date);
    }
