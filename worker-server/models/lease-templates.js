...
module.exports = {
	...
	delete:function(connection, template_id){
		var sql = "delete from lease_templates where id = " + connection.escape(template_id);
		return connection.queryAsync(sql);
	},
	...
	deleteAllServices(connection, template_id ){
		...
		var sql = "DELETE from lease_template_services where template_id = " + connection.escape(template_id);

		return connection.queryAsync(sql);
	},

	deleteService(connection, service_id){
		var sql = "UPDATE lease_template_services set status = 0 where id = " + connection.escape(service_id);
	...
	async removePaymentCycles(connection, template_id, excluded_ids ){
		var sql = `UPDATE lease_template_payment_cycles set deleted_at = NOW() where id not in (${excluded_ids.map(e => connection.escape(e)).join(',')}) and template_id = ${connection.escape(template_id) }`;
