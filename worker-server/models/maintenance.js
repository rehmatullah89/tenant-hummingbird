...
module.exports = {
    ...
    findSummaryActive:function(connection, company_id, property_id){
        var currentMaintenanceSql = "Select count(*) as count, severity" +
            " from maintenance where status = 'open' ";

            if(property_id){
                currentMaintenanceSql += " and lease_id in ( select id from leases where unit_id in ( select id from units where property_id = '" + connection.escape(property_id) + "')) ";
            }
    ...
    search(connection, conditions = {}, searchParams, company_id, count){
        ...
        if(conditions.property_id){
            sql += ' and (select property_id from units where id = (select unit_id from leases where id = maintenance.lease_id))  = ' + connection.escape(conditions.property_id);
        }
    ...
    findThread:function(connection, maintenance_id){
        var sMsgSql = "Select * from submessages where maintenance_id = " + connection.escape(maintenance_id) + " order by date DESC";
