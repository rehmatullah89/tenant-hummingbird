...
module.exports = {
    ...
    deleteInvoiceLines:function(connection, invoice_id){

        var deleteTaxSql = "DELETE from tax_line_items where invoice_line_id in (select id from invoice_lines where invoice_id = " + connection.escape(invoice_id) + ')';

        return connection.queryAsync(deleteTaxSql).then(function() {


            var deleteDiscountSql = "DELETE from discount_line_items where invoice_line_id in (select id from invoice_lines where invoice_id = " + connection.escape(invoice_id) + ')';
            return connection.queryAsync(deleteDiscountSql);
        }).then(function(){
            var deleteLineSql = "DELETE from invoice_lines where invoice_id = " + connection.escape(invoice_id);
            return connection.queryAsync(deleteLineSql);
        })
    },
    ...
    findInvoicesByLease: function(connection, lease_id, conditions, options){
        ...
        var currentInvoicesSql = "Select *, " +
            ...
            if(conditions && conditions.paid){
             ...
             currentInvoicesSql += " order by due desc ";
    ...
    processUninvoicedLines:function(connection, invoice_id, leaseId){

        var sql = "update invoice_lines set invoice_id = " + connection.escape(invoice_id) +  " where lease_id = " + connection.escape(leaseId) + " and invoice_id is null ";
    ...
    findInvoicesByPeriod: function(connection, company_id, start, end){
        ...
        var sql  = "Select * from invoices where " +
            ...
            " and due <= " + connection.escape(periodEnd.format('YYYY-MM-DD')) + " and due > " + connection.escape(periodStart.format('YYYY-MM-DD')) + " order by number asc";
    ...
    findInvoiceLinesByInvoiceId :function(connection, invoice_id){
        var invoiceLineSql = "Select * from invoice_lines where invoice_id = " + connection.escape(invoice_id);
    ...
    findMaxInvoiceNumber: function(connection, company_id){
        var sql = "Select * from invoices where lease_id in (select id from leases where unit_id in (select id from units where property_id in (select id from properties where company_id = " + connection.escape(company_id) + "))) order by CAST(number as UNSIGNED) desc limit 1";
        console.log("maxinvoice", sql);

        return connection.queryAsync(sql)
            .then(function(invoiceRes){
                if(invoiceRes.length){
                    return invoiceRes[0].number;
                } else {
                    return 0;
                }
            })
    ...
    findBalance(connection, payload) {
        ...
        with cte_balance as (
            ...
            select 
	            SUM(cte_bal.total_invoice_amount - cte_bal.payment_applied) as balance
