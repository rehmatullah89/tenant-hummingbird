  ...
  >
    ...
    <template v-slot:subheader>
      <span class="hb-text-light">
        Provide the relevant database details here
      </span>
    </template>
    <template v-slot:content>
      <v-container class="pa-5">
        ...
        <v-row class="ma-0 pa-0">
          <v-col md="12" class="pt-0 px-5 ma-0">
            <label>Read Hostname</label>
            <v-text-field
              hide-details="auto"
              class="ma-0"
              type="text"
              v-validate="'required|max:255'"
              v-model="data.read_hostname"
              required
              dense
              outlined
              id="read_hostname"
        ...
        <v-row class="ma-0 pa-0">
          <v-col md="12" class="pt-0 px-5 ma-0">
            <label>Write Hostname</label>
            <v-text-field
              hide-details="auto"
              class="ma-0"
              type="text"
              v-validate="'required|max:255'"
              v-model="data.write_hostname"
              required
              dense
              outlined
              id="write_hostname"
              name="write_hostname"
              :class="{'custom-field-error' : errors.first('write_hostname')}"
              :error-messages="errors.first('write_hostname')"
            ></v-text-field>
          </v-col>
        </v-row>
        ...
        <v-row class="ma-0 pa-0">
          <v-col md="12" class="pt-0 px-5 ma-0">
            <label>Redhsift Hostname</label>
            <v-text-field
              hide-details="auto"
              class="ma-0"
              type="text"
              v-validate="'required|max:255'"
              v-model="data.redshift_hostname"
              required
              dense
              outlined
              id="redshift_hostname"
              name="redshift_hostname"
              :class="{'custom-field-error' : errors.first('redshift_hostname')}"
              :error-messages="errors.first('redshift_hostname')"
            ></v-text-field>
          </v-col>
        </v-row>
        ...
        methods: {
            ...
            async save(){
                if(this.selected.name){
                    await api.put(this, api.DATABASES + this.data.name, this.data);
                } else {
                    await api.post(this, api.DATABASES, this.data)
                }
                this.$emit('refetch');
                this.$emit('close');
            }
