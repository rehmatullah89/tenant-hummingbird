...
class ProtectionPlan {
  ...
  constructor(data) {
    ...
    setConfig(summary) {
      ...
      this.data.data.insuranceEnrolled.push({
        key: 'total',
        name:'Total',
        unit_number: this.data.data.insuranceEnrolled.length,
      })

      this.data.data.insuranceNotEnrolled.push({
        key: 'total',
        name:'Total',
        unit_number: this.data.data.insuranceNotEnrolled.length
      })
      this.config.push({
      ...
      this.config.push({
        ...
        rows: this.data.data.insuranceEnrolled
      ...
      this.config.push({
        ...
        rows: this.data.data.insuranceNotEnrolled
      })
    }

    async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
      boldTextOptions.size = 24;
      await cxt.writeText('Insurance / Protection Enrollment', leftOffset, topOffset, boldTextOptions);
    ...
    async generate(socket){
      ...
      let data_length = this.data.data.insuranceEnrolled && this.data.data.insuranceEnrolled.length ? this.data.data.insuranceEnrolled.length : 0;

      data_length += this.data.data.insuranceNotEnrolled && this.data.data.insuranceNotEnrolled.length ? this.data.data.insuranceNotEnrolled.length : 0;
  
      let summary = [{
        total_count: data_length,
        total_percentage: utils.formatPersontage(this.data.data.insuranceSummary.percentage)
      }]
      let total_page_no = data_length % 20 === 0 ? (data_length / 20) + 1: Math.ceil(data_length / 20);
      let pcount = Math.ceil(total_page_no / 100) * 5;
     
  
      this.setConfig(summary);
      ...
      for(let i=0 ;i<this.config.length; i++) { // section loop
        ...
        if (!i) {
          ...
          topOffset = await this.printTableHeader(section,cxt,leftOffset,topOffset,padding,mediumTextOptions,boldTextOptions);
        }
        ...
        for(let j=0; j<section.rows.length; j++){
          ...
          await this.truncate_text(this.page_width,this.margin_right,total_width,cxt, section.columns,
            6 ,1 ,this.margin_left , 0 ,padding, newTopOffset + 8, textOptions, row,true);
    ...
    async printTableHeader(section,cxt, leftOffset, topOffset, padding, mediumTextOptions, boldTextOptions) {
      ...
      /*for(let k=0; k<section.columns.length; k++) {
        ...
        if(col.type === 'money' || col.text_aligin === 'right' ) {
          ...
          await cxt.writeText(col.name, leftMargin + col.tranformedWidth - textDimensions.width - padding, topOffset + 8, mediumTextOptions);
        } else {
          await cxt.writeText(col.name, leftMargin + padding, topOffset + 8, mediumTextOptions);
        }
    ...
    async truncate_text(page_width,margin_right,total_width,cxt,columns, scale_down_size ,column_width ,margin_left , leftOffset ,padding, topOffset, textOptions, d,calculate_total){
      ...
      for(let j = 0; j < columns.length; j++){
        ...
        cxt.drawPath(borderObj.start_x,borderObj.start_y,borderObj.end_x,borderObj.end_y, {color: 'red'});
        */
