...
class RentChange {
    ...
    constructor(data) {
      ...
      setConfig(){
        ...
        let rentChange = this.data.data.rentChange;
        let rentNotChange = this.data.data.rentNotChange;
        let rent_change_summary = this.data.data.rent_change_summary;

        let totalRentChange = (rentChange && rentChange.length) ? rentChange.length : 0;
        let totalRentNotChange= (rentNotChange && rentNotChange.length) ? rentNotChange.length : 0;
        ...
        this.config.push({
            ...
            rows: [ 
                {
                    "rate_categories": "<6 Months",
                    "total_count" : utils.formatNumber(rent_change_summary.six_month.count),
                    "change" : utils.formatPersontage( rent_change_summary.six_month.prct_variance ),
                    "variance": utils.formatMoney( rent_change_summary.six_month.variance )
                },
                {
                    "rate_categories": "6-12 Months",
                    "total_count" : utils.formatNumber(rent_change_summary.six_to_twelve_month.count),
                    "change" : utils.formatPersontage( rent_change_summary.six_to_twelve_month.prct_variance ),
                    "variance": utils.formatMoney( rent_change_summary.six_to_twelve_month.variance )
                    
                },
                {
                    "rate_categories": "12-18 Months",
                    "total_count" : utils.formatNumber(rent_change_summary.twelve_to_eighteen_month.count),
                    "change" : utils.formatPersontage( rent_change_summary.twelve_to_eighteen_month.prct_variance ),
                    "variance": utils.formatMoney( rent_change_summary.twelve_to_eighteen_month.variance )
                    
                },
                {
                    "rate_categories": "18-24 Months",
                    "total_count" : utils.formatNumber(rent_change_summary.eighteen_to_twenty_four_month.count),
                    "change" : utils.formatPersontage( rent_change_summary.eighteen_to_twenty_four_month.prct_variance ),
                    "variance": utils.formatMoney( rent_change_summary.eighteen_to_twenty_four_month.variance )
                    
                },
                {
                    "rate_categories": ">24 Months",
                    "total_count" : utils.formatNumber(rent_change_summary.twenty_four_month.count),
                    "change" : utils.formatPersontage( rent_change_summary.twenty_four_month.prct_variance ),
                    "variance": utils.formatMoney( rent_change_summary.twenty_four_month.variance )
                },
                {
                    "rate_categories": "Total",
                    "total_count" : utils.formatNumber(rent_change_summary.total.count),
                    "change" : utils.formatPersontage( rent_change_summary.total.prct_variance ),
                    "variance": utils.formatMoney( rent_change_summary.total.variance )
                } 
            ]
            })
        ...
        rows: [ 
            {
                "row_name": "No Rent Change in 12+ Months",
                "total_count" : utils.formatNumber(rentUnchanged.total),
                "total" : utils.formatPersontage( rentUnchanged.percentage )
        ...
        columns: [
            ...
            {
                key: 'days_rent_change',
                name:'Days Since Last Rent Change',
                type: 'string',
                width: 20
            },
            {
        ...
        this.config.push({
            name: 'Tenant with No Rent Change in 12+ Months',
            pos: 'left',
            columns: [
                {
                    key: 'name',
      ...
      async generate(socket){
        ...
        let data_length = this.data.data.rentChange && this.data.data.rentChange.length ? this.data.data.rentChange.length : 0;
        data_length += this.data.data.rentNotChange && this.data.data.rentNotChange.length ? this.data.data.rentNotChange.length : 0;
        console.log('data_length', data_length);
        let total_page_no = data_length % 50 === 0 ? (data_length / 50) + 1: Math.ceil(data_length / 50);
       console.log('total_number', total_page_no);
      ...
      async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
        ...
        await cxt.writeText('MSR Detail - Rent Change Summary', leftOffset, topOffset, boldTextOptions);
        ...
        for(let j = 0; j < columns.length; j++){
          ...
          if (textDimensions.width > column_width) {
            ...
            for(let l = 0 ; l< subRows.length; l++){
              ...
              if((columns[j].type === 'money' || columns[j].type === 'percentage' || columns[j].type === 'number' || columns[j].text_aligin === 'right')&& !calculate_total) {
          ...
          if(!textSplitFlag) {
            ...
            if(columns[j].type === 'money' || columns[j].type === 'percentage' || columns[j].type === 'number' || columns[j].text_aligin === 'right') {
      ...
      formatCell(data,col){
        switch(col.type.toLowerCase()){
          ...
          case 'boolean':
            ...
            break;
          case 'percentage':
            return utils.formatPersontage(data);
            break;
        }
      }

}
module.exports = RentChange;