...
class CashAudit {
  ...
  constructor(body) {
    ...
    this.report_notes = [
      {
        key: 'Other Fees*:',
        value: 'include Lien Fees, Recurring Fees, Other Fees'
      },
      {
        key: 'Deposits*:',
        value: 'include Security Deposits and Misc Deposits'
      },
      {
        key: 'Refunds/Void*:',
        value: 'Already calculated in the Payments and Product Type summary'
      },
      {
        key: 'NSF Reversals*:',
        value: 'Already calculated in the Payments and Product Type summary'
  ...
  async generate(socket){
    ...
    var footerOffset = 40;
    leftOffset = 0;
    topOffset -= (row_height * 2.5) + headerOffset - 1;
    let current_row = 0;
    let row = 1;
    ...
    let total_page_no = data_length % 23 === 0 ? (this.rows.length / 23) + 1: Math.ceil((this.rows.length + 4) / 23);
    ...
    for(let i = 0; i < this.rows.length; i++){
      ...
      if ( ( current_row * row_height ) + margin_bottom + margin_top + headerOffset + footerOffset  >= page_height) { // add footer height here as well
        ...
        let table_border_height = (row_height * 30) + 1;
        cxt.drawRectangle(leftOffset + 20 , 21.5 , this.page_width - 40 , table_border_height , {
          type: 'stroke',
          color: 0x808080,
          opacity: 1,
          width :0.4
        ...
        topOffset -= (row_height * 2) + headerOffset + 7;
    ...
    if (this.rows.length) {
      ...
      cxt.drawRectangle(leftOffset + 20 , topOffset - 5, this.page_width - 40 , table_border_height , {
        type: 'stroke',
        color: 0x808080,
        opacity: 1,
        width :0.4
      ...
      for(let j=0; j < this.columns.length ; j++){
        ...
        var textDimensions = textOptions.font.calculateTextDimensions(value+'',c_scale_down_size);
        textOptions.size = c_scale_down_size;

        if(this.columns[j].column_type === 'money' && textDimensions.width <= column_width) {
          ...
          summaryLeftOffset = summaryLeftOffset - summaryDateDimensions.width - (padding * 2.3);
          await cxt.writeText(value, summaryLeftOffset + margin_left + padding + 1.5, topOffset, textOptions);
        }else if (textDimensions.width > column_width) {
          var start = 0;
          var end = 0;
          end = value.length;
          ...
          switch ( true ) {
            case (c_scale_down_size == 6):
              end = char_capacity - 2;
                break;
            case (c_scale_down_size == 7):
              end = char_capacity - 6;
                break;
            case (c_scale_down_size == 8):
              end = char_capacity - 8;
                break;
            case (c_scale_down_size == 9):
              end = char_capacity - 9;
                break;
            default:
              end = char_capacity;
          ...
          if(text_lines === 1 ){
            m_top -= c_scale_down_size;
          } else if (text_lines === 2 ) {
              if(!calculate_total){
                m_top = (c_scale_down_size >= 6 ? -c_scale_down_size/2 : -c_scale_down_size); 
              } else{
                m_top = (c_scale_down_size >= 6 ? -c_scale_down_size/2 : -2); 
              }
          } else if(text_lines >= 3) {
            m_top = 0;
          }

          let newTopOffset = topOffset + c_scale_down_size + m_top;
          ...
          for(let l = 0 ; l< subRows.length; l++){
            ...
            if((columns[j].column_type === 'money' || columns[j].column_type === 'number')&& !calculate_total) {
              ...
              let moneyTextDimensions = font.calculateTextDimensions(subRows[l],c_scale_down_size);
              newCeilLeftOffSet -= moneyTextDimensions.width;
            }

            await cxt.writeText(subRows[l].trim(), newCeilLeftOffSet , newTopOffset, textOptions);
            newTopOffset = newTopOffset - c_scale_down_size - 0.7;
        ...
        if (j === 1 || j === 6 || j === 7 || j === 19 || j === 20) {
          let borderTop = topOffset + 12.5;
      ...
      //If not enough space for Notes, go to the next page
      if(( current_row * row_height ) + margin_bottom + margin_top + headerOffset + footerOffset + 77 >= page_height){
        ...
        // Reprint Headers
        textOptions.font = boldFont;
        await this.printHeaders(cxt, [], margin_left + leftOffset, topOffset, titleTextOptions,textOptions,font,padding,page_width,margin_right,pdfWriter);
        textOptions.font = font;
        
        topOffset -= headerOffset - 1;
        page_number++
      }else {
        topOffset -= 30;
      }
      ...
      await cxt.writeText('Note:', leftOffset ,topOffset, txtMedOpt);
      ...
      let table_border_height = row_height + 2.5;
  ...
  async truncate_text(page_width,margin_right,total_width,pdfWriter,cxt,columns, scale_down_size ,column_width ,margin_left , leftOffset ,padding, topOffset, textOptions, d, total,calculate_total){
    ...
    for(let j = 0; j < columns.length; j++){
      ...
      if ((j === 2 ) && !calculate_total) {
        ...
        await cxt.writeText('Payments', newLeftOffSet + column_width, topOffset + 17, titleTextOptions);

      } else if ((j === 11 || j === 18 || j === 6 || j === 0 || j === 5 || j === 19)) {
          ...
          if (j === 10 && !calculate_total ) {
            await cxt.writeText('Product Types', newLeftOffSet + padding + 25, topOffset + 17, titleTextOptions);
          }else if(j === 18 || j === 6 || j === 0 || j === 5 || j === 19) { 
            borderLeft = calculate_total ? borderLeft += 20 : borderLeft; 
            if(j === 4 || j === 18) borderTop -= 19.8
            else borderTop ;
            borderObj = {
              start_x: borderLeft,
              start_y: borderTop,
              end_x: borderLeft,
              end_y: borderTop -  (calculate_total ? j !== 4 && j !== 18 ? 38.5 : 19 : 41)
            }
