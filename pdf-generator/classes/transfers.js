...
class Transfers {
  ...
  constructor(body) {
    ...
    this.report_name = 'Transfer Report';
  ...
  setConfig(){
    ...
    this.data.data.forEach(row => {
      this.rows.push({
        ...row,
        transfer_balance: row.in_balance - row.out_balance
      });
    });
  }
  ...
  async generate(socket){
    ...
    var footerOffset = 40;
    leftOffset = 0;
    topOffset -= (row_height * 2) + headerOffset - 1;
    let current_row = 0;
    let row = 1;
    ...
    let total_page_no = data_length % 23 === 0 ? (this.rows.length / 23) + 1: Math.ceil(this.rows.length / 23);
    ...
    for(let i = 0; i < this.rows.length; i++){
      ...
      if ( ( current_row * row_height ) + margin_bottom + margin_top + headerOffset + footerOffset  >= page_height) { // add footer height here as well
        ...
        cxt.drawRectangle(leftOffset + 20 , 26.5 , this.page_width - 40 , table_border_height , {
          type: 'stroke',
          color: 0x808080,
          opacity: 1,
          width :0.4
  ...
  async truncate_text(page_width,margin_right,total_width,pdfWriter,cxt,columns, scale_down_size ,column_width ,margin_left , leftOffset ,padding, topOffset, textOptions, d, total,calculate_total){
    ...
    for(let j = 0; j < columns.length; j++){
      ...
      if (j === 0 && !calculate_total) {
        ...
        await cxt.writeText('Transfer Out', newLeftOffSet + column_width, topOffset + 17, titleTextOptions);
        borderTop += 32.4;
        borderObj = {
          start_x: borderLeft,
          start_y: borderTop,
          end_x: borderLeft,
          end_y: borderTop - 19.7
        }
        ...
        if(calculate_total) {
          ...
          titleTextOptions = {
           ...
           if(j === 6) {
            await cxt.writeText('Transfer In', newLeftOffSet + padding + 25, topOffset + 17, titleTextOptions);
          } else  if (j === 11 ) {
            await cxt.writeText('Details', newLeftOffSet + padding + 25, topOffset + 17, titleTextOptions);
          }
        }
        ...
        borderObj = {
          ...
          start_y: borderTop,
          end_x: borderLeft,
          end_y: borderTop - (calculate_total ? 21 : 39.7)
        }
