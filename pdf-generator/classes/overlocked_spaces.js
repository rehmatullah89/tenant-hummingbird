...
class OverlockedSpaces {
    ...
    constructor(data) {
      ...
      setConfig(){
        ...
        let ovelockedSpaces = this.data.data.ovelockedSpaces;
        let totalovelockedSpaces = this.data.data.summary.total;
        //let notOverlockedSpaces = this.data.data.notOverlockedSpaces;
        //let totalnotOverlockedSpaces = (notOverlockedSpaces && notOverlockedSpaces.length) ? notOverlockedSpaces.length : 0;
        //let data_length = totalovelockedSpaces + totalnotOverlockedSpaces;

        let totalOvelockedSpacesPercentage = this.data.data.summary.percentage;

        let total_amount_ovelocked = ovelockedSpaces.reduce(function(tot, arr) {
            return tot + arr.balance;
        }, 0);

        // let total_amount_not_ovelocked = notOverlockedSpaces.reduce(function(tot, arr) {
        //     return tot + arr.balance;
        // }, 0);
  


        ovelockedSpaces.push(
          {
            key: 'Total',
            name: 'Total',
            space_number: utils.formatNumber(totalovelockedSpaces) + "",
            balance: total_amount_ovelocked,
            isfooter: true
          }
        )

        // notOverlockedSpaces.push(
        //   {
        //     key: 'Total',
        //     name: 'Total',
        //     space_number: utils.formatNumber(totalnotOverlockedSpaces) + "",
        //     balance: total_amount_not_ovelocked.toFixed(2),
        //     isfooter: true
        //   }
        // )
        ...
        rows: [ 
            {
                "total_count" : utils.formatNumber(totalovelockedSpaces) + "",
                "total" : utils.formatPersontage( totalOvelockedSpacesPercentage ),
            } 
        ]
        })
        ...
        rows: ovelockedSpaces
        })

        // this.config.push({
        //     name: 'Not Overlocked Spaces',
        //     pos: 'left',
        //     columns: [
        //         {
        //             key: 'name',
      ...
      async generate(socket){
        ...
        let data_length = this.data.data.ovelockedSpaces && this.data.data.ovelockedSpaces.length ? this.data.data.ovelockedSpaces.length : 0;
      ...
      async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
        ...
        await cxt.writeText('MSR Detail - Overlocked Spaces', leftOffset, topOffset, boldTextOptions);
      ...
      async PrintNetSummary(cxt, leftOffset, topOffset, mediumTextOptions) {
        ...
        mediumTextOptions.size = 9;
        let str =  `Total Auto Pay Enrolled's: ${this.data.data.autopayEnrolled.length}`;
        ...
        mediumTextOptions.size = 9;
        str =  `Total Auto Pay Not Enrolled's: ${this.data.data.autopayNotEnrolled.length}`;
        ...
        mediumTextOptions.size = 9;
        str =  `Net: ${this.data.data.autopayEnrolled.length - this.data.data.autopayNotEnrolled.length}`;
...
module.exports = OverlockedSpaces;