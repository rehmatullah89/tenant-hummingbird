...
class NetRevenue {
    ...
    constructor(data) {
      ...
      setConfig(){
        ...
        let rent = this.data.data.rent;
        let fee = this.data.data.fee;
        let product = this.data.data.product;
        let insurance = this.data.data.insurance;

        let totalRent = (rent && rent.length) ? rent.length : 0;
        let totalFee = (fee && fee.length) ? fee.length : 0;
        let totalProduct = (product && product.length) ? product.length : 0;
        let totalInsurance = (insurance && insurance.length) ? insurance.length : 0;

        let data_length = totalRent + totalFee + totalProduct + totalInsurance;

        let total_amount_rent = rent.reduce(function(tot, arr) {
            return tot + arr.amount;
        }, 0);
        let total_amount_fee = fee.reduce(function(tot, arr) {
            return tot + arr.amount;
        }, 0);
        let total_amount_product = product.reduce(function(tot, arr) {
            return tot + arr.amount;
        }, 0);
        let total_amount_insurance = insurance.reduce(function(tot, arr) {
            return tot + arr.amount;
        }, 0);
        ...
        rent.push(
          {
            key: 'Total',
            name: 'Total',
            unit_number: utils.formatNumber(totalRent),
        ...
        fee.push(
          {
            key: 'Total',
            name: 'Total',
            unit_number: utils.formatNumber(totalFee),
        ...
        insurance.push(
            {
              key: 'Total',
              name: 'Total',
              unit_number: utils.formatNumber(totalInsurance),
        ...
        rows: [ 
            {
                "revenue_name" : "Rent",
                "total" : total_amount_rent 
            },
            {
                "revenue_name" : "Insurance / Protection",
                "total" : total_amount_insurance
            },
            {
                "revenue_name" : "Fee",
                "total" : total_amount_fee
            },
            {
                "revenue_name" : "Merchandise",
                "total" : total_amount_product
            },
            {
                "revenue_name" : "Total",
                "total" : total_amount_rent + total_amount_fee + total_amount_product + total_amount_insurance, 
                isfooter: true
            }
        ]
        })
      ...
      async generate(socket){
        ...
        for(let i=0 ;i<this.config.length; i++) { // section loop
          ...
          if (!i) {
            ...
            topOffset = await this.printTableHeader(section,cxt,leftOffset,topOffset + 10,padding - 3,mediumTextOptions,boldTextOptions);
          }
          ...
          for(let j=0; j<section.rows.length; j++){//loop rows
            ...
            if(row.key === 'Total' || row.key === 'total'){
            ...
            }else if(row.key === 'Total' || row.key === 'total'){
                cxt.drawRectangle(leftOffset + 0.3, newTopOffset + 0.4 , this.page_width - this.margin_right - this.margin_left - 0.6, row_height -0.4, {
                  type: 'fill',
                  color: 0xDFE3E8,
                  opacity: 0.9
      ...
      async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
        ...
        await cxt.writeText('MSR Detail - Net Revenue and Projected Income', leftOffset, topOffset, boldTextOptions);
      ...
      async printTableHeader(section,cxt, leftOffset, topOffset, padding, mediumTextOptions, boldTextOptions) {
        ...
        cxt.drawRectangle(leftOffset, topOffset + 10 , this.page_width - this.margin_left - this.margin_right , 10 , {
          type: 'stroke',
          color: 0x808080,
          opacity: 1,
          width :0.4
