...
class GenericReport {
    ...
    async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
      ...
      await cxt.writeText(this.title, leftOffset, topOffset, boldTextOptions);
      ...
      if (startDate !== endDate) {
        if (this.end_date !== null && this.end_date !== undefined) report_date += ` - ${endDate}`
      }
      ...
      if (this.property.Emails.length) {
        ...
        await cxt.writeText(this.property.Emails[0].email, facilityLeftOffset , topOffset - emailOffset , mediumTextOptions);
      }
    };
