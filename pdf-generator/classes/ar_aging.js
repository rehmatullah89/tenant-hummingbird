...
class AccountReceivableAging {
  ...
  constructor(body) {
    ...
    this.report_name = 'AR Aging';
    this.report_date = body.start_date;
    ...
    if (this.data.length) {
      ...
      for(let j=0; j < this.columns.length ; j++){
        ...
        if(this.columns[j].column_type === 'money') {
          ...
          let summaryDateDimensions = font.calculateTextDimensions(value,c_scale_down_size);
          summaryLeftOffset = summaryLeftOffset - summaryDateDimensions.width - (padding * 2);
          await cxt.writeText(value, summaryLeftOffset + margin_left + padding, topOffset, textOptions);
        }
  ...
  async printHeaders(cxt, header_columns, startingLeftOffset, topOffset, titleTextOptions,textOptions,font,padding,page_width,margin_right, pdfWriter){
    ...
    let report_date = moment(this.report_date).format('dddd, MMMM Do YYYY');
...
module.exports = AccountReceivableAging;
