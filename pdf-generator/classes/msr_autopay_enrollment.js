...
class MsrAutopayEnrollment {
    ...
    constructor(data) {
      ...
      setConfig(){
        ...
        let autopayEnrolled = this.data.data.autopayEnrolled;
        let autopayNotEnrolled = this.data.data.autopayNotEnrolled;

        let totalAutoPayEnrolled = (autopayEnrolled && autopayEnrolled.length) ? autopayEnrolled.length : 0;
        let totalAutoPayNotEnrolled = (autopayNotEnrolled && autopayNotEnrolled.length) ? autopayNotEnrolled.length : 0;
      
        //let data_length = totalAutoPayEnrolled + totalAutoPayNotEnrolled;

        let summary = this.data.data.autopaySummary;

        autopayEnrolled.push(
          {
            key: 'Total',
            name: 'Total',
            space_number: utils.formatNumber(totalAutoPayEnrolled),
            isfooter: true
          }
        )

        autopayNotEnrolled.push(
          {
            key: 'Total',
            name: 'Total',
            space_number: utils.formatNumber(totalAutoPayNotEnrolled),
            isfooter: true
          }
        )
        ...
        rows: [ 
            {
                "total_count" : utils.formatNumber(summary.total),
                "total" : utils.formatPersontage( summary.percentage )
            } 
        ]
        })
        ...
        rows: autopayEnrolled
        })

        this.config.push({
            name: 'Autopay / Not Enrolled',
            pos: 'left',
            columns: [
              {
                key: 'name',
      ...
      async generate(socket){
        ...
        let data_length = this.data.data.autopayEnrolled && this.data.data.autopayEnrolled.length ? this.data.data.autopayEnrolled.length : 0;
        data_length += this.data.data.autopayNotEnrolled && this.data.data.autopayNotEnrolled.length ? this.data.data.autopayNotEnrolled.length : 0;
      ...
      async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
        ...
        await cxt.writeText('MSR Detail - AutoPay Enrollment ', leftOffset, topOffset, boldTextOptions);
...
module.exports = MsrAutopayEnrollment;