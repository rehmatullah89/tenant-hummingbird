...
class AccountantSummary {
  constructor(body){
    ...
    this.data = body.data || [];
    this.columns = [];
    this.report_name = 'Accountant Summary';
    ...
    this.data.map((d,i) =>{
      ...
      d.event_sections.forEach((sec,j) => {
        ...
        this.data[i].event_sections[j].columns = [
          { key: "gl_account", width :(( 70 *  ( this.page_width - this.margin_right  - this.margin_left) ) / total_width), value : `${sec.event_category} - ${sec.book_type} book`, column_type:"string" },
        ...
        this.data[i].property_rows += 3; //section header and footer and space b/w tables
        this.data[i].property_rows += this.data[i].event_sections[j].entries.length;
      });

      this.data[i].property_rows -= 1; // total spaces = total tables - 1;
      this.data[i].page_count = this.data[i].property_rows % 41 === 0 ? (this.data[i].property_rows / 41) : Math.ceil(this.data[i].property_rows / 41);
  ...
  async printHeader(property, cxt,pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
    ...
    await cxt.writeText('Accountant Summary', leftOffset, topOffset, boldTextOptions);
