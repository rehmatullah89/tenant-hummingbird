...
class WriteOffs {
    ...
    constructor(data) {
      ...
      setConfig(){
        ...
        let writeOffs = this.data.data.writeOffs;


        let total_amount = writeOffs.reduce(function(tot, arr) {
        ...
        writeOffs.push(
            {
              ...
              name: 'Total',
              summary_column: true,
              space_number: utils.formatNumber(writeOffs.length),
              amount: total_amount,
              isfooter: true
            }
          )
      ...
      async generate(socket){
        ...
        let data_length = this.data.data.writeOffs && this.data.data.writeOffs.length ? this.data.data.writeOffs.length : 0;
      ...
      async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
        ...
        await cxt.writeText('MSR Detail - Write Offs Report', leftOffset, topOffset, boldTextOptions);
