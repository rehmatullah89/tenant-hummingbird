...
class OccupancySummary {
    ...
    constructor(data) {
      ...
      setConfig(){
        ...
        let occupiedList = this.data.data.occupiedList;
        let vacantList = this.data.data.vacantList;

        let totalOccupiedList = (occupiedList && occupiedList.length) ? occupiedList.length : 0;
        let totalVacantList = (vacantList && vacantList.length) ? vacantList.length : 0;
      
        let data_length = totalOccupiedList + totalVacantList;

        let summary = this.data.data.occupancy_breakdown;

        let total_occupied_area = occupiedList.reduce(function(tot, arr) {
            return tot + arr.area;
        }, 0);

        let total_vacant_area = vacantList.reduce(function(tot, arr) {
            return tot + arr.area;
        }, 0);
        ...
        occupiedList.push(
          {
            key: 'Total',
            name: 'Total',
            space_number: utils.formatNumber(totalOccupiedList),
            area: total_occupied_area,
            isfooter: true
          }
        )
        ...
        vacantList.push(
          {
            key: 'Total',
            name: 'Total',
            space_number: utils.formatNumber(totalVacantList),
            area: total_vacant_area,
            isfooter: true
          }
        )
        ...
        rows: [ 
            {
                ...
                "total_count" : utils.formatNumber(summary.occupied.unit_count),
                "space_percentage" : utils.formatPersontage( summary.occupied.unit_percent ),
                "sq_tf" : utils.formatMoney(summary.occupied.sqft,false),
                "total" : utils.formatPersontage(summary.occupied.sqft_percent)

            },
            {
                "unit_status" : "Vacant",
                "total_count" : utils.formatNumber(summary.vacant.unit_count),
                "space_percentage" : utils.formatPersontage( summary.vacant.unit_percent),
                "sq_tf" : utils.formatMoney(summary.vacant.sqft,false),
                "total" : utils.formatPersontage(summary.vacant.sqft_percent)

            },
            {
                "unit_status" : "Total",
                "total_count" : utils.formatNumber(summary.occupied.unit_count + summary.vacant.unit_count),
                "space_percentage" : utils.formatPersontage( summary.occupied.unit_percent + summary.vacant.unit_percent),
                "sq_tf" : utils.formatMoney(summary.occupied.sqft + summary.vacant.sqft, false),
                "total" : utils.formatPersontage(summary.occupied.sqft_percent + summary.vacant.sqft_percent),
                isfooter: true
      ...
      async generate(socket){
        ...
        let data_length = this.data.data.occupiedList && this.data.data.occupiedList.length ? this.data.data.occupiedList.length : 0;
        data_length += this.data.data.vacantList && this.data.data.vacantList.length ? this.data.data.vacantList.length : 0;
      ...
      async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
        ...
        await cxt.writeText('MSR Detail - Space Occupancy', leftOffset, topOffset, boldTextOptions);
...
module.exports = OccupancySummary;