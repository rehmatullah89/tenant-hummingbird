...
class AppliedCredits {
    ...
    constructor(data) {
      ...
      setConfig(){
        ...
        let appliedCredits = this.data.data.appliedCredits;


        let total_amount = appliedCredits.reduce(function(tot, arr) {
        ...
        appliedCredits.push(
            {
              ...
              name: 'Total',
              summary_column: true,
              space_number: utils.formatNumber(appliedCredits.length),
              amount: total_amount,
              isfooter: true
            }
          )
      ...
      async generate(socket){
        ...
        let data_length = this.data.data.appliedCredits && this.data.data.appliedCredits.length ? this.data.data.appliedCredits.length : 0;
      ...
      async printHeader(cxt, pdfWriter, leftOffset, topOffset, mediumTextOptions, boldTextOptions) {
        ...
        await cxt.writeText('MSR Detail - Applied Credits Report', leftOffset, topOffset, boldTextOptions);
      ...
      async PrintNetSummary(cxt, leftOffset, topOffset, mediumTextOptions) {
        ...
        mediumTextOptions.size = 9;
        let str =  `Total Write Off's: ${this.data.data.writeOffs.length}`;
        ...
        mediumTextOptions.size = 9;
        str =  `Net: ${this.data.data.writeOffs.length - this.data.data.writeOffs.length}`;
