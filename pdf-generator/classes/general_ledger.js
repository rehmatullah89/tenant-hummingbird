...
class GeneralLedger {
  constructor(body){
    ...
    this.data = body.data || [];
    this.columns = [];
    this.report_name = 'General Ledger';
  ...
  sortData(){
    for(let i=0;i<this.data.length;i++){
      this.data[i].report_data.sort((a, b) => {
  ...
  async generate(socket){
    ...
    for(let i=0;i<this.data.length;i++){
      if(!this.data[i].report_data || (this.data[i].report_data && this.data[i].report_data.length === 0)){
        total_page_no++;
      } else {
        let data_length = this.data[i].report_data.length;
        total_page_no += data_length % 40 === 0 ? (data_length / 40) + 1: Math.ceil(data_length / 40);
      }
    }
    ...
    for(let k = 0; k < this.data.length; k++){
      ...
      for(let i = 0; i < this.data[k].report_data.length; i++){
        let d = this.data[k].report_data[i];
        ...
        if ( ( current_row * row_height ) + margin_bottom + margin_top + headerOffset + footerOffset  >= page_height) { // add footer height here as well
          ...
          let table_border_height = (row_height * 41 ) + 1.5;
          cxt.drawRectangle(leftOffset + 20 , 35 , this.page_width - 40 , table_border_height , {
            type: 'stroke',
            color: 0x808080,
            opacity: 1,
            width :0.4
          ...
          await cxt.writeText(printText, margin_left,15, textOptions);
      ...
      if (this.data[k].report_data.length) {
  ...
  async printHeaders(cxt, header_columns, startingLeftOffset, topOffset, titleTextOptions,textOptions,font,padding,page_width,margin_right, pdfWriter){
    ...
    if(this.logoUrl) {
      ...
      cxt.drawImage(this.page_width - this.margin_right - 95, topOffset - logoTopOffset ,this.logoUrl,imageOptions);
    } else {
