...
class CoverageDetails {
    constructor(body) {
        ...
        this.report_name = 'Coverage Details Report';
    ...
    setConfig() {
        this.columns = [
            {
                key: 'property_name', // adding column
                label: 'Property', // to be displayed in pdf
                column_type: 'string',// used for formatting 
                width: 30,
            },
            {
                key: 'space_number',
            ...
            {
                key: 'tenant_address_1',
                label: 'Address 1',
                column_type: 'string',
                width: 25
            },
            {
                key: 'tenant_address_2',
                label: 'Address 2',
                column_type: 'string',
                width: 25
            },
            ...
            {
                key: 'insurance_refunded_amount',
                label: 'Refunded Amount',
                column_type: 'money',
                width: 15
            },
            {
                key: 'insurance_credited_amount',
                label: 'Credited Amount',
                column_type: 'money',
                width: 15
            },
            {
                key: 'insurance_paid_amount',
                label: 'Paid Amount',
                column_type: 'money',
                width: 15
            },
    ...
    async generate(socket) {
        ...
        let page_width = this.page_width;
        let page_height = this.page_height;

        let ws = new ms.WritableStream()
        let pdfWriter = hummus.createWriter(new hummus.PDFStreamForResponse(ws));

        let font = pdfWriter.getFontForFile(settings.config.base_path + 'fonts/Graphik-Regular.ttf');
        let mediumFont = pdfWriter.getFontForFile(settings.config.base_path + 'fonts/Graphik-Medium.ttf');
        let boldFont = pdfWriter.getFontForFile(settings.config.base_path + 'fonts/Graphik-Bold.ttf');
        ...
        for (let i = 0; i < this.data.length; i++) {
            let d = this.data[i];
            let array_of_string = Object.values(d);
            array_of_string = array_of_string.filter(Boolean);
            array_of_string = array_of_string.map(item => item.toString());
            let string_value = await this.longest_string(array_of_string);
            let textDimensions = font.calculateTextDimensions(string_value[0], 9);
        ...
        for (let i = 0; i < this.data.length; i++) {
            ...
            if ((current_row * row_height) + margin_bottom + margin_top + headerOffset + footerOffset >= page_height) { // add footer height here as well
                page_number += 1;
                // outer table
                let table_border_height = (row_height * 28) + 5;
        ...
        if (this.data.length) {
            let table_border_height = (row_height * (current_row + 1)) + 6;
            ...
            let table_border_height = (row_height * 2) + 10;
    ...
    async printHeaders(cxt, header_columns, startingLeftOffset, topOffset, titleTextOptions, textOptions, font, padding, page_width, margin_right, pdfWriter) {
        ...
        let semiBoldFont = pdfWriter.getFontForFile(settings.config.base_path + 'fonts/Graphik-Semibold-Regular.ttf');
        ...
        if (this.property.id) {
            ...
            let boldFont = semiBoldFont;
            let facilityNameDimensions = boldFont.calculateTextDimensions(facility_name, 9);
            let facilityLeftOffset = this.page_width - this.margin_right - squareSize - facilityNameDimensions.width - 8;

            titleTextOptions.size = 9;
            titleTextOptions.color = 'black';
            await cxt.writeText(facility_name, facilityLeftOffset, topOffset + 6.5, titleTextOptions);

            let mediumFont = textOptions.font;
            textOptions.size = 7;
            textOptions.color = 'black';

            let facilityAddressDimensions = mediumFont.calculateTextDimensions(facility_address, 7);
            ...
            let facilityPhoneDimensions = mediumFont.calculateTextDimensions(facility_phone, 7);
            ...
            let companyNameDimensions = font.calculateTextDimensions(this.company.name, 9);
            let companyLeftOffset = this.page_width - this.margin_right - squareSize - companyNameDimensions.width;
            titleTextOptions.color = 'black';
        ...
        let header_width_array = [];

        for (let i = 0; i < header_columns.length; i++) {

            let titleDimensions = font.calculateTextDimensions(header_columns[i].value, 9);
            let h_width = Math.floor((header_columns[i].width / titleDimensions.width) * 9);
    ...
    async truncate_text(page_width, margin_right, total_width, pdfWriter, cxt, columns, scale_down_size, column_width, margin_left, leftOffset, padding, topOffset, textOptions, d, total, calculate_total) {
        ...
        let font = calculate_total ? pdfWriter.getFontForFile(settings.config.base_path + 'fonts/Gotham-Medium-Regular.ttf') : textOptions.font;
        ...
        for (let j = 0; j < columns.length; j++) {
            ...
            let textDimensions = font.calculateTextDimensions(value + '', scale_down_size);
    ...
    async longest_string(array_of_string) {
        let max = array_of_string[0].length;
        array_of_string.map(v => max = Math.max(max, v.length));
        let result = array_of_string.filter(v => v.length == max);
        return result;
    }
